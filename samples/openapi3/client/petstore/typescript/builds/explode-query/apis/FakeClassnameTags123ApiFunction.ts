...
*/
async function testClassnameRequestProcessor(options: Configuration,client: Client, ): Promise<RequestContext> {
    let _config = options;
...
*/
async function testClassnameWithHttpInfoResponseProcessor(response: ResponseContext): Promise<HttpInfo<Client >> {
...
export async function testClassnameWithHttpInfo(
    ...
    const requestContext = await testClassnameRequestProcessor(configuration,client);
    const response = await configuration.httpApi.send(requestContext).toPromise();

    return await testClassnameWithHttpInfoResponseProcessor(response);
}
