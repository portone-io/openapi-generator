...
[RequestProcessor]
*/
async function listRequestProcessor(options: Configuration,): Promise<RequestContext> {
    let _config = options;
...
*/
async function listWithHttpInfoResponseProcessor(response: ResponseContext): Promise<HttpInfo<ListPaged >> {
...
export async function listWithHttpInfo(
    ...
    const requestContext = await listRequestProcessor(configuration,);
    const response = await configuration.httpApi.send(requestContext).toPromise();

    return await listWithHttpInfoResponseProcessor(response);
}
